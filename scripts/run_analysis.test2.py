...
logging.info("\t\t- [%s] GATK CollectInsertSizeMetrics ..." % time.strftime("%H:%M:%S"))
for barcode in ordered_barcodes:
	logging.info("\t- %s :" % (barcodes_json[barcode]['sample']))
	cism_folder = '%s/cism' % barcodes_json[barcode]['intermediate_folder']
	subprocess.call(['mkdir',cism_folder])
	cmd = subprocess.Popen(['docker','exec','-it','gatk','gatk','CollectInsertSizeMetrics','-I','%s' % barcodes_json[barcode]['bam'],'-O','%s/%s.CollectInsertSizeMetrics.txt' % (cism_folder,barcodes_json[barcode]['sample']),'-H','%s/%s.Histogram.txt' % (cism_folder,barcodes_json[barcode]['sample'])], stdout=open('%s/CollectInsertSizeMetrics.stdout.txt' % cism_folder,'w'), stderr=open('%s/CollectInsertSizeMetrics.stderr.txt' % cism_folder,'w'))
