...
if not os.path.isdir(conta_folder):
	...
	#subprocess.call(['mkdir', results_folder + '/_2018_reanalysis/'])
	#subprocess.call(['mkdir', results_folder + '/_2018_reanalysis/check-contamination'])
...
cmd = subprocess.Popen(['python','/DATA/work/custom_run_analysis.py','--bam','%s/%s_%s.filtered.sorted.bam' % (conta_folder,sample,barcode),'--run-type',run_type],stdout=subprocess.PIPE)
...
for i in range(1,len(contaSheet_rows)):
	if contaSheet_rows[i][2].value:
		variantSearch[(contaSheet_rows[i][2].value,contaSheet_rows[i][5].value)] = [] #(NM,c)
...
for s in sample2compare:
	finalReport_path = '%s/_2018_reanalysis/%s/%s_%s.finalReport.xlsx' % (run_folder,s[0],s[0],s[1])
	if os.path.isfile(finalReport_path):
		...
		for i in range(1,len(annoSheet_rows)):
			if (annoSheet_rows[i][2].value,annoSheet_rows[i][5].value) in variantSearch.keys():
				variantSearch[(annoSheet_rows[i][2].value,annoSheet_rows[i][5].value)].append(s[0]+'_'+s[1])
...
for i in range(1,len(contaSheet_rows)):
	if contaSheet_rows[i][2].value:
		...
		if variantSearch[contaSheet_rows[i][2].value,contaSheet_rows[i][5].value]: # list not empty = variant found somewhere
			foundstring = ','.join(variantSearch[(contaSheet_rows[i][2].value,contaSheet_rows[i][5].value)])
...
subprocess.call(['cp',conta_folder+'/Check-contamination_%s.xlsx'%sample,run_folder+'/_2018_reanalysis/_Check-contamination/Check-contamination_%s.xlsx'%sample])
