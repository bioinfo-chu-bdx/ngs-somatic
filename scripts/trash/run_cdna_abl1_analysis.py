...
reference = global_param['abl1_ref']
...
for bamfile in sorted(bamlist) :
	...
	# dossier results
	results_name = bamfile.split('/')[-1].replace(' ','@') # (ne devrait plus etre necessaire), variant_caller_pipeline.py plante si espace dans output_dir
	results_folder = '/DATA/work/results/' + results_name
	if not os.path.isdir(results_folder):
		subprocess.call(['mkdir', results_folder])
	
	bam_data[bamfile] = {'barcode':barcode,'sample':sample,'run_type':run_type,'sample_folder':sample_folder,'results_folder':results_folder,'target_bed':target_bed,'bed_merged':bed_merged,'param':param,'param_hotspot_only':param_hotspot_only,'hotspot_vcf':hotspot_vcf,'ds_file':ds_file}
...
for bamfile in sorted(bamlist) :
	...
	cmd = subprocess.Popen(['python','/DATA/work/variantAnnotation/abl1_merge_annovar_avinput.py',results_folder], stdout=open(results_folder+'/merge_annovar_avinput.stdout.txt','w'), stderr=open(results_folder+'/merge_annovar_avinput.stderr.txt','w'))
	out, err = cmd.communicate()
	...
	cmd = subprocess.Popen(['python', '/DATA/work/variantAnnotation/VEP/vep_abl1.py', results_folder+'/variants.merged.avinput', results_folder+'/format_vep.tsv'], stdout=open(results_folder+'/vep.stdout.txt','w'), stderr=open(results_folder+'/vep.stderr.txt','w'))
	out, err = cmd.communicate()
	...
	cmd = subprocess.Popen([
		'python','/DATA/work/finalReport/abl1_finalReport.py',
...
if options.full_run and os.path.isfile(run_folder+'/barcodes.json'):
	...
	for controlbam in checkconta_bamlist:
		cmd = subprocess.Popen(['python','/DATA/work/checkContamination/abl1_Check_contamination.py','--bam', controlbam,'--read-len', '100',], stdout=subprocess.PIPE)
		out, err = cmd.communicate()
