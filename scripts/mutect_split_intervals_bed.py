...
i = 0
for chunk in chunks :
	chunk_file_name = '%s.%04d-scattered.intervals' % (bed_name,i)
	chunk_file = open('%s/reference_files/mutect/%s' % (pipeline_folder,chunk_file_name),'w')
	...
	chunk_file.close()
	i+=1
print "\n- chunks files writted in %s/reference_files/mutect/" % pipeline_folder
