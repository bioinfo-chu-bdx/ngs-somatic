...
TOOLDIR=$NGS_PIPELINE_BX_DIR/CNV/oncocnv/		#set a path to the directory with CNV detection scripts. In your case, it can be, for example, ONCOCNV.v5.5/	
OUTDIR=$4          					#set a path to the *existing* directory to write the output
GENOME=$NGS_PIPELINE_BX_DIR/reference_files/hg19/hg19.fasta        #set a path to the .fasta file with the reference genome
