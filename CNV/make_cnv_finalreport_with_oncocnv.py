...
def cell_format(cell, copy, amp_colors, del_colors, alignment='center', border=None):
	...
	if copy == 'insufficient' or copy == '.':
		color = 'Grey'
	elif copy == 'unchanged':
		color = 'LightGrey'
	elif not isinstance(copy, float):
		color = 'Red'		
	else:		
		if copy >= 3.0:
...
CNV_report_path = "%s/CNV/CNV_Report_Empty_with_oncocnv.xlsx" % pipeline_folder
...
### IONCOPY GAIN RESULTS per amplicon
column2sample = {}
...
### IONCOPY LOSS RESULTS per amplicon
column2sample = {}
...
### ONCOCNV RESULTS
oncocnv_sample_gene_cn = {}
for sample in column2sample.values():
	oncocnv_sample_gene_cn[sample] = {}
	oncsearch = glob.glob('%s/oncocnv/%s_*.summary.txt' % (cna_folder,sample))
	try:
		oncocnv_summary_file = open(oncsearch[0],'r')
	except:
		print "***WARNING*** ONCOCNV file not found for %s" % sample
		continue
	oncocnv_reader = csv.reader(oncocnv_summary_file,delimiter='\t')
	oncocnv_reader.next()
	for line in oncocnv_reader:
		if line[0] not in oncocnv_sample_gene_cn[sample]:
			if line[5] == 'NA': 
				print "***WARNING*** NA p-value for %s - %s" % (sample,line[0])
				oncocnv_sample_gene_cn[sample][line[0]] = 'NA'
			else:
				oncocnv_sample_gene_cn[sample][line[0]] = float(line[4])
		else: 
		### BUG GENE EN DOUBLE, en general une occurence a la p-value 'NA' et il faut prendre l'autre
			if line[5] == 'NA': 
				print "***WARNING*** Duplicate found in summary, ignoring line with NA p-value for %s - %s" % (sample,line[0])
			else:
				if oncocnv_sample_gene_cn[sample][line[0]] == 'NA':
					print "*** replacing NA value with correct one for %s - %s" % (sample,line[0])
					oncocnv_sample_gene_cn[sample][line[0]] = float(line[4])
				else:
					value = str(oncocnv_sample_gene_cn[sample][line[0]]).replace('(','').replace(')','') + '-' + str(float(line[4]))
					value = '(' + value + ')'
					oncocnv_sample_gene_cn[sample][line[0]] = value
					print "***WARNING : DUP *** Duplicate found in summary, both valable p-value for %s - %s" % (sample,line[0])
	
	
# COMBINE IONCOPY RESULTS
ioncopy_sample_gene_cn = {}
for sample in sample_amplicon_cn.keys():
	ioncopy_sample_gene_cn[sample] = {}
	for amplicon in sample_amplicon_cn[sample].keys():
		gene = amplicon.split('_')[0]
		if gene not in ioncopy_sample_gene_cn[sample].keys():
			ioncopy_sample_gene_cn[sample][gene] = []
		...
		if gain_value == 2.0 and loss_value == 2.0:
			ioncopy_sample_gene_cn[sample][gene].append(2.0)
		elif gain_value == 2.0:
			ioncopy_sample_gene_cn[sample][gene].append(loss_value)
		elif loss_value == 2.0:
			ioncopy_sample_gene_cn[sample][gene].append(gain_value)
		else:
...
gene_index = 2
gene2column = {}
for gene in ioncopy_sample_gene_cn[sample].keys(): # a partir du dernier "sample" en variable
	results_sheet.cell(row=1,column=gene_index).value = gene
	results_sheet.cell(row=2,column=gene_index).value = 'IONCOPY'
	results_sheet.cell(row=2,column=gene_index+1).value = 'ONCOCNV'
	gene2column[gene] = gene_index
	gene_index = gene_index + 2

sample2row = {}
for sample in ioncopy_sample_gene_cn.keys():
	...
	for gene in ioncopy_sample_gene_cn[sample].keys():
		# IONCOPY
		mean_copy = sum(ioncopy_sample_gene_cn[sample][gene])/len(ioncopy_sample_gene_cn[sample][gene])
		if mean_copy == 2.0:
			results_sheet.cell(row=current_row,column=gene2column[gene]).value = "unchanged"
			cell_format(results_sheet.cell(row=current_row,column=gene2column[gene]), "unchanged", amp_colors, del_colors)
		else:
			results_sheet.cell(row=current_row,column=gene2column[gene]).value = round(mean_copy,1)
			cell_format(results_sheet.cell(row=current_row,column=gene2column[gene]), mean_copy, amp_colors, del_colors)
		#ONCOCNV
		oncocnv_copy = oncocnv_sample_gene_cn[sample].get(gene, ".")
		if oncocnv_copy == '.':
			results_sheet.cell(row=current_row,column=gene2column[gene]+1).value = "."
			cell_format(results_sheet.cell(row=current_row,column=gene2column[gene]+1), ".", amp_colors, del_colors, border="medium")
		elif oncocnv_copy == 2.0:
			results_sheet.cell(row=current_row,column=gene2column[gene]+1).value = "unchanged"
			cell_format(results_sheet.cell(row=current_row,column=gene2column[gene]+1), "unchanged", amp_colors, del_colors, border="medium")
		else:
			results_sheet.cell(row=current_row,column=gene2column[gene]+1).value = oncocnv_copy
			cell_format(results_sheet.cell(row=current_row,column=gene2column[gene]+1), oncocnv_copy, amp_colors, del_colors, border="medium")
	current_row = current_row + 1


### LIGNE GRISEE mauvais sequencage en utilisant donnees du ioncopy_input#####################
