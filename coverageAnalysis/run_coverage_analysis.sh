...
if [ $# -ne 2 ]; then
  echo "$CMD: Invalid number of arguments."
  echo -e "$USAGE\n$OPTIONS"
  exit 1;
fi
...
BBCTOOLS="/home/t8illumy/ngs-pipeline-bx/coverageAnalysis/bbctools"
if [ ! -f "$BBCTOOLS" ]; then
  BBCTOOLS="bbctools"
...
if [ $SHOWLOG -eq 1 ]; then
  echo -e "\n$CMD BEGIN:" `date`
  echo "(`date`) $CMD started."
  echo "REFERENCE: $REFERENCE"
  echo "MAPPINGS:  $BAMNAME"
  echo "GENOME:    $GENOME"
  if [ -n "$BEDFILE" ]; then
    echo "TARGETS:   $BEDFILE"
  fi
  if [ -n "$STATSFILE" ];then
...
if [ "$ANNOBEDFORMAT" -ne 0 -a $NOTARGETANAL -eq 0 ];then
  ...
  if [ -n "$INBEDFILE" ]; then
    echo "(`date`) Creating GC annotated targets file..."
...
if [ $BASECOVERAGE -eq 1 ]; then
  ...
  if [ $PADVAL -gt 0 ];then
    if [ $TRACK -eq 1 ]; then
      echo "(`date`) Creating base coverage files for padded targets..."
    fi
...
if [ $TRACK -eq 1 ]; then
  if [ -n "$BASECOVOPTS" ]; then
    if [ -n "$TARGETCOVOPTS" ]; then
      echo "(`date`) Creating base coverage files and analyzing $TARGETTYPE coverage..."
    else
      echo "(`date`) Creating base coverage files..."
    fi
  elif [ -n "$TARGETCOVOPTS" ]; then
    echo "(`date`) Analyzing $TARGETTYPE coverage..."
  fi
...
if [ $DOSORT -ne 0 ]; then
  if [ $TRACK -eq 1 ]; then
    echo "(`date`) Sorting $TARGETTYPE coverage results to increasing read depth order..."
  fi
...
if [ $BASECOVERAGE -eq 1 ]; then
  if [ $NOTARGETANAL -ne 0 ];then
    echo "(`date`) Skipping generating contig coverage files..."
  elif [ $RNABED -eq 0 ];then
    if [ $TRACK -eq 1 ]; then
      echo "(`date`) Generating contig coverage files..."
    fi
    TRGOPTS=""
...
if [ $BASECOVERAGE -eq 1 ]; then
  if [ $NOTARGETANAL -ne 0 ];then
    echo "(`date`) Skipping coverage overview analysis..."
  elif [ $RNABED -eq 0 ];then
    if [ $TRACK -eq 1 ]; then
      echo "(`date`) Calculating effective reference coverage overview plot..."
    fi
...
if [ -n "$TRACKINGBED" ]; then
  if [ $TRACK -eq 1 ]; then
    echo "(`date`) Analyzing sample tracking reads..."
  fi
...
if [ $TRACK -eq 1 ]; then
  echo "(`date`) Analyzing depth of coverage..."
fi
...
eval "$STATCMD >> '$STATSFILE'"
if [ $? -ne 0 ]; then
  echo -e "\nERROR: targetReadStats.pl failed." >&2
  echo "\$ $STATCMD >> '$STATSFILE'"
  exit 1;
fi
...
if [ $NOTARGETANAL -ne 0 ]; then
  echo "(`date`) Skipping fine coverage analysis..."
elif [ -n "$ANNOBEDOPT" ]; then
  if [ $TRACK -eq 1 ]; then
    echo "(`date`) Generating start-up $TARGETTYPE coverage chart data..."
  fi
  ...
  if [ $PROPPLOTS -eq 1 ]; then
    if [ $TRACK -eq 1 ]; then
      echo "(`date`) Generating static representation plots..."
    fi
    ...
    if [ $? -ne 0 ]; then
      ...
      echo "> $TARGETCOV_REP_PNG"
      echo "> $TARGETCOV_GC_PNG"
      if [ -e "$TARGETCOV_RPP_PNG" ];then
        echo "> $TARGETCOV_RPP_PNG"
      fi
      if [ $REPLENPLOT -eq 1 ]; then
        echo "> $TARGETCOV_RPL_PNG"
        echo "> $TARGETCOV_LEN_PNG"
      fi
    fi
  fi
  if [ $RNABED -eq 1 ]; then
    if [ $TRACK -eq 1 ]; then
      echo "(`date`) Generating amplicon representation overview plot..."
    fi
    if [ $CONTIGS -ne 0 ];then
      REPOVRTRG='Contigs'
    else
...
if [ -n "$RESHTML" ]; then
  ...
  echo -e "(`date`) Creating HTML report..."
  GENOPT="-g"
  if [ -n "$BEDFILE" ]; then
    GENOPT=""
  ...
  if [ $NOTARGETANAL -ne 0 ]; then
    ...
    echo "(`date`) Skipping output for $TARGETREP representation and coverage chart..."
  elif [ -n "$ANNOBED" ]; then
  ...
  if [ $SHOWLOG -eq 1 ]; then
    echo "HTML report complete: " `date`
  fi
fi
